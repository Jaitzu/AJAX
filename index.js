
// pistetään hakukenttä toimimaan myös enter painikkeella
const input = document.getElementById("etsi");
input.addEventListener("keyup" , (event)=>{
    event.preventDefault();
    if (event.keyCode === 13){
        myFunction();
    }
});


//inputista saatu tieto
function myFunction() {
    let x = document.getElementById("etsi").value;
    let text = 'http://api.tvmaze.com/search/shows?q=' +   x ;
console.log(text);
    fetch(text )             // Käynnistetään haku. Vakiometodi on GET.
        .then(function (vastaus) {        // Sitten kun haku on valmis,
            return vastaus.json();        // muutetaan ladattu tekstimuotoinen JSON JavaScript-olioksi
        }).then( (json)=> {         // Sitten otetaan ladattu data vastaan ja
        naytaKuva(json);              // kutsutaan naytaKuva-funktiota ja lähetetään ladattu data siihen parametrinä.
    }).catch(function (error) {       // Jos tapahtuu virhe,
        console.log(error);           // kirjoitetaan virhe konsoliin.
    });
}
function naytaKuva(tv) {
    const tulos = document.getElementById('tulokset');
    tulos.innerHTML = "";
    console.log(tv);
    for (let i = 0; i < tv.length; i++) {

        // valitaan oikea sisältö
        const nimi = tv[i].show.name;
        const linkki = tv[i].show.url;
        const lin = 'Linkki';
        let kuva;
        if(tv[i].show.image===null){
             kuva = 'data:image/png;base64,';
        }
        else {
             kuva = tv[i].show.image.medium;
        }
        const genre = tv[i].show.genres;
        const veto = tv[i].show.summary;
        console.log(nimi);
        console.log(linkki);
        console.log(kuva);
        console.log(genre);
        console.log(veto);

        // luodaan sivun elementit
        const body = document.getElementById('tulokset');

        //luodaan ohjelman tiedoille oma kotelo
        const kotelo = document.createElement('div');
        kotelo.setAttribute('id', 'kotelo');
        body.appendChild(kotelo);

        //kotelo kuvalle
        const kuvakotelo = document.createElement('div');
        kuvakotelo.setAttribute('class', 'picture');
        kotelo.appendChild(kuvakotelo);

        //tekstinkotelo
        const tekstikotelo = document.createElement('div');
        tekstikotelo.setAttribute('class', 'text');
        kotelo.appendChild(tekstikotelo);

        //luodaan elementit
        const img = document.createElement('img');
        kuvakotelo.appendChild(img);
        const h = document.createElement('h1');
        tekstikotelo.appendChild(h);
        const p = document.createElement('p');
        tekstikotelo.appendChild(p);
        const div = document.createElement('div');
        tekstikotelo.appendChild(div);
        const a = document.createElement('a');
        tekstikotelo.appendChild(a);

        //lisätään elementteihin sisältö
        img.alt ="ei vittu ole";
        img.src = kuva;
         h.innerText = nimi;
        p.innerHTML = '<p> Genres: </p>'  + genre + '<br>';
        div.innerHTML = '<p> Summary: </p> ' + veto;
        a.href = linkki;
        a.innerText = lin;

        console.log('haku valmis');

/*        document.getElementById("kotelo").addEventListener("mouseover", mouseOver);
        document.getElementById("kotelo").addEventListener("mouseout", mouseOut);

        function mouseOver() {
            document.getElementById("kotelo").setAttribute('class', 'puna');
        }

        function mouseOut() {
            document.getElementById("kotelo").setAttribute('class', 'sini');
        }*/



}

}
